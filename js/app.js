...
function setView(viewName) {
    appRoot.innerHTML = ''; // Clear current screen
    
    switch(viewName) {
        case 'home': renderHome(); break;
        case 'books': 
            if (typeof renderBooksHome === 'function') renderBooksHome(); 
            break;
        case 'quiz': 
...
// Renders the glowing cards using Premium SVG Icons
function renderHome() {
    appRoot.innerHTML = `
...
// Initialize the app and bind the Chat UI
window.onload = () => {
    // 1. Load the home grid
    renderHome();
    
    // 2. Activate the AI Chat Input
    ...
    if (sendBtn && chatInput && chatStream) {
        ...
        const sendMessage = () => {
            ...
            if (text) {
                ...
                userMsg.style.alignSelf = 'flex-start'; // User on left, AI on right
                userMsg.style.borderBottomLeftRadius = '4px';
                userMsg.style.borderBottomRightRadius = '20px';
                userMsg.style.color = '#a0a5b5'; // Slightly faded text for user
                userMsg.textContent = text;
                
                // Add to stream and auto-scroll
